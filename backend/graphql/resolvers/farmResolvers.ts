...
const farmResolvers = {
  Query: {
    farms: async (
      _parent: undefined,
      { filter, pageNumber, pageSize }: { filter?: FarmFilter; pageNumber: number; pageSize: number },
      context: AuthContext
    ) => {
      ...
      if (!isAdmin) {
        return farmService.getFarms(pageNumber, pageSize, { ...filter, status: FarmStatus.APPROVED });
      }
