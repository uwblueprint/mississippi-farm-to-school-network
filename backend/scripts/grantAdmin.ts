...
async function main() {
  ...
  if (!email) {
    console.error('Usage: npx tsx scripts/grantAdmin.ts <email> [password]');
    process.exit(1);
  }
  ...
  await auth.setCustomUserClaims(user.uid, { ...(user.customClaims ?? {}), role: Role.ADMIN });
  console.log(`Set custom claim role=ADMIN on ${user.uid}`);

  const users = getFirestore().collection(Collections.users);
  const doc = await users.doc(user.uid).get();
  const now = new Date().toISOString();
  if (doc.exists) {
    await users.doc(user.uid).update({ role: Role.ADMIN, is_verified: true, updatedAt: now });
    console.log(`Updated Firestore users/${user.uid} to role=ADMIN`);
  } else {
  ...
  console.log(
    `Done. ${email} is an admin. An already-signed-in session must sign out and back in to pick up the new claim.`
  );
}
