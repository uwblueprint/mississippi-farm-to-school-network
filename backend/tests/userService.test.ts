...
describe('UserService.updateUserById', () => {
  ...
  test('updates email without allowing role changes', async () => {
    ...
    expect(existingUser.update).toHaveBeenCalledWith({ email: 'new@example.com' });
  ...
  test('does not call Firebase when the email address is unchanged', async () => {
    ...
    expect(existingUser.update).toHaveBeenCalledWith({ email: 'old@example.com' });
    expect(result.role).toBe(Role.ADMIN);
  });
});
