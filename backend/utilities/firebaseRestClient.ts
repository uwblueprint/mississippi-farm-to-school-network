...
const FirebaseRestClient = {
  ...
  signInWithPassword: async (email: string, password: string): Promise<Token> => {
  ...
  signInWithGoogleOAuth: async (idToken: string): Promise<OAuthSignInResponse> => {
    const response: Response = await fetch(
    ...
    const responseJson: OAuthSignInResponse | RequestError = await response.json();
  ...
  refreshToken: async (refreshToken: string): Promise<Token> => {
    ...
    const responseJson: RefreshTokenResponse | RequestError = await response.json();
